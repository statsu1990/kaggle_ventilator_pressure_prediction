...
class Model:
    def __init__(
        ...
        self.nrnb = VentilatorNearestNeighbors(nn_prms, remove_nearest=False)
    ...
    def fit(self, tr_x, tr_y, vl_x, vl_y):
        ...
        # fit nn
        self.nrnb.fit(tr_x[:, :self.use_seq_len, 0], tr_y[:, :self.use_seq_len])
        score = self.nrnb.score(vl_x[:, :self.use_seq_len, 0], tr_y[:, :self.use_seq_len])
    ...
    def predict(self, x):
        ...
        # nbnr predicting
        pred = self.nrnb.predict(x[:, :self.use_seq_len, 0])
