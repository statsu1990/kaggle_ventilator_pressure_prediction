...
class VentilatorNearestNeighbors:
    ...
    def average_kneighbors(self, x):
        ...
        """
        y_neighbor = self.kneighbors(x, return_x=False)
        av_neighbor = y_neighbor.mean(axis=1)

        return av_neighbor

    def kneighbors(self, x, return_x=False):
        """
        Args:
            x (pd.DataFrame): (sample, feature)
        """
        ...
        # (sample * n_neighbors, seq, 1)
        y_neighbor = self.y[ind].copy()
        # (sample, n_neighbors, seq, 1)
        y_neighbor = y_neighbor.reshape([x.shape[0], -1, y_neighbor.shape[1], 1])

        if return_x:
            # (sample * n_neighbors, seq)
            x_neighbor = self.x[ind].copy()
            # (sample, n_neighbors, seq)
            x_neighbor = x_neighbor.reshape([x.shape[0], -1, x_neighbor.shape[1]])
